...
# Load the 87 exact raw quartic ambiguity identities recovered by the
# independently certified quartic run.
def parse_app(a):
 typ=a[0];lead=tuple(a[1]);rem=tuple(a[2]) if isinstance(a[2],list) else int(a[2]);return (typ,lead,rem)
raw=[(tuple(z[0]),parse_app(z[1])) for z in json.load(open('y36_quartic_raw_relations.json'))]
assert len(raw)==87
...
for x in range(maxx):
 for ri,(m,alt) in enumerate(raw):
  ...
  if len(cache5)>250000:
   cache5.clear();gc.collect();print(f'cache cleared after generator {tested5}',flush=True)
